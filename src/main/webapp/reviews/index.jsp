                ...
                <c:forEach var="review" items="${reviews}">
                    <div class="m-2">
                        <div class="rounded-lg bg-[#2F4156] shadow-lg w-80 h-auto">
                            <c:if test="${not empty review.movie.image}">
                                <img src="<c:url value='/storage/${review.movie.image}' />"
                                     alt="${review.movie.title} Image"
                                     class="rounded-t-lg object-cover h-96 w-full">
                            </c:if>
                            <c:if test="${empty review.movie.image}">
                            ...
                            <div class="p-4 text-white">
                                ...
                                <p class="mb-2 text-sm">Release Date: ${review.movie.releaseDate}</p>
                                <p class="mb-4 text-sm">${fn:substring(review.review, 0, 100)}</p>
                                <p class="mb-4 text-sm">Rating: ${review.rating}/5</p>
                                <a href="<c:url value='/reviews/${review.id}' />"
